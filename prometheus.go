...
func (conf MiddlewareConfig) ToMiddleware() (ghttp.HandlerFunc, error) {
	...
	return func(r *ghttp.Request) {
		...
		reqSz := computeApproximateRequestSize(r.Request)
		...
		if conf.AfterNext != nil {
			conf.AfterNext(r, err)
		}

		url := r.Router.Uri // contains route path ala `/users/:id`
		if url == "" || url == "/*" {
		...
		responseSize.WithLabelValues(values...).Observe(float64(r.Response.BufferLength()))
	}, nil
}
	...
	for {
		...
		case <-ticker.C:
			...
			if res.StatusCode != http.StatusOK {
				if hErr := config.ErrorHandler(fmt.Errorf("post metrics request did not succeed")); hErr != nil {
					return hErr
				}
			}
		case <-ctx.Done():
